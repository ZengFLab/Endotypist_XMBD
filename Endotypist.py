...
class Endotypist(nn.Module):
    """
    This class encapsulates the parameters (neural networks) and models & guides needed to train a
    deep random-effects model on single cell datasets
    """
    ...
    def model(self, xs, ys=None, ks=None, ks2=None):
        ...
        with pyro.plate('data'):
            ...
            zns = pyro.sample('zn', dist.Normal(
                prior_loc, prior_scale).to_event(1))
...
def main(args):
    """
    run inference for Endotypist

    :param args: arguments for Endotypist
    :return: None
    """
    ...
    if args.aux_loss:
        ...
        loss_aux = SVI(scc.model_classify, scc.guide_classify,
                       scheduler, loss=elbo)
...
EXAMPLE_RUN = (
    "example run: python Endotypist.py --seed 0 --cuda -n 300 "
    "--sup-data-file <sup_data_file> "
    "--sup-condition-file <wanted_covariate_file> "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="the label file of the supervised data that describes cell type annotation",
    )
    parser.add_argument(
        "--sup-condition-file",
        default=None,
        type=str,
        help="the condition file of the supervised data that describes desired covariates, such as disease condition",
    )
    parser.add_argument(
        "--sup-condition2-file",
        default=None,
        type=str,
        help="the condition file of the supervised data that describes unwanted covariates, such as batch effect",
    )
    parser.add_argument(
        "--unsup-data-file",
        default=None,
